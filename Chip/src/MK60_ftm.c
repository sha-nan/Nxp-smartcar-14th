 ...
 *  Sample usage:       FTM_PWM_init(FTM0, FTM_CH6,200, 10);    //初始化 FTM0_CH6 为 频率 200Hz 的PWM，占空比为 10/FTM0_PRECISON 10000u
 */
...
void     ftm_input_clean (FTMn_e ftmn)//清计数器计数值
{
  ASSERT( ftmn < FTM_MAX);     //检查传递进来的通道是否正确
  FTM_CNT_REG(FTMN[ftmn]) = 0;          //清计数器计数值
}
